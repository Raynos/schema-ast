...
function HttpResponse(res) {
    ...
    if (res.body) {
        opts.body = isSchemaBranded(res.body) ?
            res.body : V.struct(res.body);
    }

    return V.struct(opts);
}

function HttpTypedError(typedError, extra) {
    assert(typedError, 'typedError is required');

    var keys = Object.keys(typedError);

    assert(keys &&
        keys.length > 0,
        'expected typedError to have properties');
    assert(typeof typedError.type === 'string',
        'expected `typedError.type` to be a string');
    assert(typeof typedError.statusCode === 'number',
        'expected `typedError.statusCode` to be a number');

    var opts = {
        type: V.string(typedError.type),
        name: V.string(),
        message: V.string(),
        statusCode: V.number(typedError.statusCode)
    };

    if (extra) {
        assert(typeof extra === 'object',
            'expected `extra` to be an object');

        opts = extend(opts, extra);
    }
